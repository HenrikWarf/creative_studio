document.addEventListener('DOMContentLoaded', () => {
    ...
    setupContextAccordion('btn-context-accordion-vm-fl', 'context-content-vm-fl', 'context-checkboxes-vm-fl', 'btn-apply-context-vm-fl', 'vm-fl-context', 'vm-fl-context-version');
    ...
    if (vmFlCountSlider && vmFlCountDisplay) {
        vmFlCountSlider.addEventListener('input', (e) => {
            vmFlCountDisplay.textContent = e.target.value;
        });
    }
    ...
    // Helper for First Frame Upload
    if (dropZoneVmFlFirst && fileInputVmFlFirst) {
        dropZoneVmFlFirst.addEventListener('click', () => fileInputVmFlFirst.click());
        dropZoneVmFlFirst.addEventListener('dragover', (e) => {
            e.preventDefault();
            dropZoneVmFlFirst.style.borderColor = 'var(--accent-color)';
        });
        dropZoneVmFlFirst.addEventListener('dragleave', () => {
            if (!vmFlFirstFile) dropZoneVmFlFirst.style.borderColor = 'rgba(255, 255, 255, 0.1)';
        });
        dropZoneVmFlFirst.addEventListener('drop', (e) => {
            e.preventDefault();
            dropZoneVmFlFirst.style.borderColor = 'rgba(255, 255, 255, 0.1)';
            if (e.dataTransfer.files.length) handleVmFlFirstFile(e.dataTransfer.files[0]);
        });
        fileInputVmFlFirst.addEventListener('change', (e) => {
            if (e.target.files.length) handleVmFlFirstFile(e.target.files[0]);
        });
    }
    ...
    if (btnRemoveImgVmFlFirst) {
        btnRemoveImgVmFlFirst.addEventListener('click', (e) => {
            e.stopPropagation();
            vmFlFirstFile = null;
            fileInputVmFlFirst.value = '';
            previewImgVmFlFirst.src = '';
            previewImgVmFlFirst.onclick = null;
            previewContainerVmFlFirst.style.display = 'none';
            dropZoneVmFlFirst.style.display = 'flex';
        });
    }

    // Helper for Last Frame Upload
    if (dropZoneVmFlLast && fileInputVmFlLast) {
        dropZoneVmFlLast.addEventListener('click', () => fileInputVmFlLast.click());
        dropZoneVmFlLast.addEventListener('dragover', (e) => {
            e.preventDefault();
            dropZoneVmFlLast.style.borderColor = 'var(--accent-color)';
        });
        dropZoneVmFlLast.addEventListener('dragleave', () => {
            if (!vmFlLastFile) dropZoneVmFlLast.style.borderColor = 'rgba(255, 255, 255, 0.1)';
        });
        dropZoneVmFlLast.addEventListener('drop', (e) => {
            e.preventDefault();
            dropZoneVmFlLast.style.borderColor = 'rgba(255, 255, 255, 0.1)';
            if (e.dataTransfer.files.length) handleVmFlLastFile(e.dataTransfer.files[0]);
        });
        fileInputVmFlLast.addEventListener('change', (e) => {
            if (e.target.files.length) handleVmFlLastFile(e.target.files[0]);
        });
    }
    ...
    if (btnRemoveImgVmFlLast) {
        btnRemoveImgVmFlLast.addEventListener('click', (e) => {
            e.stopPropagation();
            vmFlLastFile = null;
            fileInputVmFlLast.value = '';
            previewImgVmFlLast.src = '';
            previewContainerVmFlLast.style.display = 'none';
            dropZoneVmFlLast.style.display = 'flex';
        });
    }
    ...
    if (btnOptimizeVmFl) {
        btnOptimizeVmFl.addEventListener('click', async () => {
            // For first/last, we could optimize based on instructions + maybe first image?
            // Reusing the generic prompt optimizer for now, or the image-aware one if first image exists.
            ...
            try {
                ...
                if (vmFlFirstFile) {
                    ...
                    // Text-only optimization (we don't have this endpoint exposed separately in this file, 
                    // but we can assume user might want to provide image later. 
                    // Let's just use the optimize-with-image endpoint if we have an image, 
                    // otherwise maybe we should fail or add a text-only endpoint?
                    // Actually, let's just alert user to upload an image for best results, or implement text-only optimization.
                    // For now, I'll restrict to having an image for enhancement as it gives better results.
                    showAlert('Please upload the First Frame to allow AI to see what you are working with.');
    ...
    if (btnGenerateVmFl) {
        btnGenerateVmFl.addEventListener('click', async () => {
            if (!vmFlFirstFile || !vmFlLastFile) {
                showAlert('Please upload both First and Last frames.');
                return;
            }
            if (!vmFlPrompt.value) {
                showAlert('Please enter a prompt.');
            ...
            vmFlResultContainer.innerHTML = `
                <div class="loading-state">
                    <i class="fa-solid fa-spinner fa-spin" style="font-size: 2rem; color: var(--accent-color);"></i>
                    <p style="margin-top: 1rem;">Generating transition video with Veo...</p>
                </div>
            `;
            ...
            if (vmFlContext.value) {
                formData.append('context', vmFlContext.value);
            }
            if (vmFlCountSlider) {
                formData.append('num_videos', vmFlCountSlider.value);
            }

            try {
                const response = await fetch('/api/video-magic/first-last', {
                    method: 'POST',
                    body: formData
                });
                    ...
                    data.videos.forEach((video, index) => {
                        ...
                        card.innerHTML = `
                            ...
                            <div class="video-actions" style="display: flex; gap: 0.5rem; justify-content: center; margin-top: 0.5rem;">
                                ...
                                <a href="${video.video_url}" download="generated-transition-${index}.mp4" class="action-btn">
                                    <i class="fa-solid fa-download"></i>
                                </a>
                            </div>
                        `;
